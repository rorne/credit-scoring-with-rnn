...
def transform_credits_to_sequences(
    credits_frame: pd.DataFrame, num_last_credits: int = 0
) -> pd.DataFrame:
    """
    ...
    """
    return (
        credits_frame.sort_values(["id", "rn"])
        .groupby(["id"])[features]
...
def create_padded_buckets(
    ...
    """
    frame_of_sequences["sequence_length"] = frame_of_sequences["sequences"].apply(
        lambda x: len(x[1])
    )
    frame_of_sequences["bucket_idx"] = frame_of_sequences["sequence_length"].map(
        bucket_info
    )
    padded_seq = []
    targets = []
    ...
    for size, bucket in frame_of_sequences.groupby("bucket_idx"):
        padded_sequences = (
            bucket["sequences"].apply(lambda x: pad_sequence(x, size)).values
        )
