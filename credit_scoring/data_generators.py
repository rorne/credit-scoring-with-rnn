...
def batches_generator(
    ...
    if output_format not in ["torch", "tf"]:
        raise ValueError(
            'Unknown format. Please choose one of the following formats: "torch", "tf"'
        )
        ...
        for path in list_of_paths:
            ...
            for idx in range(len(ids)):
                ...
                for jdx in range(0, len(bucket), batch_size):
                    ...
                    if output_format == "tf":
                        batch_sequences = [
                            batch_sequences[:, i] for i in range(len(features))
                        ]
                        ...
                        batch_sequences = [
                            torch.LongTensor(batch_sequences[:, i]).to(device)
                        ...
                        if is_train:
                            ...
                            yield dict(id_=batch_ids, features=batch_sequences)
        if not is_infinite:
            break
